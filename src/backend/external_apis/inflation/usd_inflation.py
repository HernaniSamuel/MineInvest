...
class USDInflationAPI(InflationAPIInterface):
    ...
    def _fetch_and_cache_all_cpi(self) -> None:
        ...
        try:
            ...
            # Accept both 'date' and 'observation_date' as date column
            date_idx = None
            ...
            if date_idx is None:
                raise ValueError(f"Date column not found in CSV. Headers: {header}")

            if 'cpiaucsl' not in header_lower:
                raise ValueError(f"'cpiaucsl' column not found in CSV. Headers: {header}")

            cpi_idx = header_lower.index('cpiaucsl')
            ...
            for row in csv_reader:
                if len(row) <= max(date_idx, cpi_idx):
                    continue  # Skip incomplete rows

                date_str = row[date_idx].strip()
                cpi_value = row[cpi_idx].strip()

                # Skip rows with missing data (marked as ".")
                if not date_str or not cpi_value or cpi_value == ".":
                    continue

                # Parse date (FRED format: "YYYY-MM-DD")
